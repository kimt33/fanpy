...
def amsgrad(objective, x0, learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-8,
            maxiter=1000, counter_limit=10, xtol=1e-8, gtol=1e-8, ftol=1e-8, lower_limit=-np.infty):
    ...
    while niter < maxiter and nsuccess < counter_limit:
        ...
        f = func(x)
        # if abs(f - f_prev) < 1e-5 or f < lower_limit or niter % 5 == 0:
        #     objective.adapt_pspace()
        #     m = np.zeros(nparams)
        #     v = np.zeros(nparams)
