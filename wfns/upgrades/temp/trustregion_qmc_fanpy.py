...
def _minimize_trustregion_constr(fun, x0, args, grad,
    ...
    if schrodinger.energy_bound != -np.inf:
        constraints.append(
            scipy.optimize.NonlinearConstraint(
                objective.fun, schrodinger.energy_bound, 0,
...
class SystemEquationsSystem(SystemEquations):
    ...
    def save_params(self):
        if self.tmpfile != "":
            ...
            np.save('{}_um{}'.format(header, ext), self.ham._prev_unitary)
...
def minimize(
    objective, norm_constraint=True, constraint_bounds=(-0.1, 0.1), energy_bound=-np.inf,
    save_file="", **kwargs
):
    objective, old_objective = SystemEquationsSystem(
    ...
    objective.energy_bound = energy_bound
