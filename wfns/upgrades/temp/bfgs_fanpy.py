...
def equality_constrained_sqp(fun_and_constr, grad_and_jac, lagr_hess,
    ...
    if (
        schrodinger.wfn in schrodinger.param_selection._masks_container_params and
        schrodinger.param_selection._masks_container_params[schrodinger.wfn].size
    ):
        wfn = schrodinger.wfn
        param_indices = schrodinger.param_selection._masks_objective_params[wfn]
        param_indices = np.where(param_indices)[0]
        wfn_indices = schrodinger.param_selection._masks_container_params[wfn]
    else:
        wfn = None
    if (
        schrodinger.ham in schrodinger.param_selection._masks_container_params and
        schrodinger.param_selection._masks_container_params[schrodinger.ham].size
    ):
        ham = schrodinger.ham
        ham_indices = schrodinger.param_selection._masks_objective_params[ham]
    ...
    while not stop_criteria(state, x, last_iteration_failed,
        ...
        if reduction_ratio >= SUFFICIENT_REDUCTION_RATIO:
            ...
            header, ext = os.path.splitext(schrodinger.tmpfile)
            np.save(header + '_pspace' + ext, schrodinger.pspace)
            np.save(header + '_refwfn' + ext, schrodinger.refwfn)
            np.save(header + '_pspace_norm' + ext, schrodinger.wfn.pspace_norm)
            np.save(header + '_eqn_weights' + ext, schrodinger.eqn_weights)
...
def tr_interior_point(fun, grad, lagr_hess, n_vars, n_ineq, n_eq,
    ...
    # XXXX: overwrite objectives to fit fanpy API
    ...
    while True:
        ...
        if 'norm' in schrodinger.adapt_type:
            schrodinger.wfn.update_pspace_norm()
...
def _minimize_trustregion_constr(fun, x0, args, grad,
    ...
    if method == 'equality_constrained_sqp':
        ...
        _, result = tr_interior_point(
            ...
            initial_constr_penalty, initial_tr_radius,
            factorization_method, schrodinger)
...
class OneSidedEnergySystem(OneSidedEnergy):
    def __init__(self, objective):
        super().__init__(
            objective.wfn, objective.ham, tmpfile=objective.tmpfile,
            param_selection=objective.param_selection, refwfn=objective.refwfn
        )
...
class SystemEquationsSystem(SystemEquations):
    def __init__(self, objective):
        super().__init__(
            objective.wfn,
            objective.ham,
            tmpfile=objective.tmpfile,
            param_selection=objective.param_selection,
        ...
        if (
            len(objective.constraints) > 1 or
            not all(isinstance(cons, NormConstraint) for cons in objective.constraints)
        ):
            raise ValueError(
        ...
        if objective.constraints:
            if not hasattr(self.wfn, 'pspace_norm'):
                self.wfn.pspace_norm = set(objective.constraints[0].refwfn)
        else:
...
class NormConstraintSystem(NormConstraint):
    def __init__(self, objective):
        ...
        self.param_selection = objective.param_selection
        self.tmpfile = ''
...
def minimize(objective, constraint_bounds=(-1e-2, 1e-2), save_file="", **kwargs):
    ...
    try:
        ...
        objective.sample_size = objective.params.size // 10
    try:
    ...
    kwargs = {"method": "trust-constr", "jac": energy.gradient}
    kwargs.setdefault("options", {"gtol": 1e-8, "disp": 2, 'schrodinger': objective})
    ...
    param_bounds = ((-2, 2) for _ in range(objective.params.size))
    ...
    output = wrap_scipy(minimize_rewritten)(
        energy, save_file=save_file, constraints=constraints, bounds=param_bounds, **kwargs
    )
