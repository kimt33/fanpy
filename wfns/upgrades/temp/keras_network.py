...
class KerasNetwork(BaseWavefunction):
    ...
    def get_overlaps(self, sds, deriv=None):
        if len(sds) == 0:
            return np.array([])
        occ_vectors = np.zeros((len(sds), self.nspin))
        occ_vectors[
            np.arange(len(sds))[:, None], np.array([slater.occ_indices(sd) for sd in sds])
        ] = 1

        scale = self.output_scale
        vals = np.hstack(self.model.predict(occ_vectors))
        if deriv is None:
            return np.prod(vals * scale, axis=1)

        raise NotImplementedError('Gradients are accumulated in tensorflow by construction')
