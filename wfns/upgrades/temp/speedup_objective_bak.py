...
from cext_apg import _olp_deriv_internal, _olp_deriv_internal_ap1rog
...
def _olp_deriv(self, sd):
    ...
    return _olp_deriv_internal(orbpair_generator, self.params, self.dict_orbpair_ind)
...
old_integrate_sd_wfn = RestrictedChemicalHamiltonian.integrate_sd_wfn
...
def integrate_sd_wfn2(self, sd, wfn, wfn_deriv):
    ...
    output += np.sum(
        self._integrate_sd_sds_zero(occ_alpha, occ_beta)[:, :, None] * overlaps_zero,
        axis=1
    )
    ...
    output += np.sum(integrals_one_alpha[:, :, None] * overlaps_one_alpha, axis=1) + np.sum(
        integrals_one_beta[:, :, None] * overlaps_one_beta, axis=1
    )
...
def wrapped_get_overlap(self, sd, deriv=None):
    ...
    d_overlaps = self.wfn.get_overlap(sd, True)
...
def wrapped_integrate_wfn_sd(self, sd, deriv=None):
    ...
    output = np.zeros(self.params.size)
    try:
        output[self.param_selection._masks_objective_params[self.wfn]] = np.sum(
            self.ham.integrate_sd_wfn(sd, self.wfn, wfn_deriv=True), axis=0
        )
    except KeyError:
        pass
    try:
        output[self.param_selection._masks_objective_params[self.ham]] = np.sum(
            self.ham.integrate_sd_wfn_deriv(sd, self.wfn, np.arange(ham_nparams)), axis=0
        )
    except KeyError:
        pass
    return output
...
def objective_system(self, params, return_energy=False, normalize=True):
    params = np.array(params)
    # Assign params to wavefunction, hamiltonian, and other involved objects.
    self.assign_params(params)
    # Normalize
    if normalize:
        self.wfn.normalize(self.pspace)
    # Save params
    ...
    if hasattr(self, 'adaptive_weights') and self.adaptive_weights:
        self.update_weights(overlaps, obj)
...
def jacobian_system(self, params, return_d_energy=False, normalize=True, adaptive=False):
    params = np.array(params)
    # # Assign params
    # self.assign_params(params)
    # Normalize
    ...
    if self.energy_type in ["variable", "fixed"]:
        ...
        if self.energy_type == "variable":
            ...
            d_energy = self.param_selection._masks_objective_params[self.energy].astype(float)
        else:
...
def update_weights(self, overlaps, obj):
    ...
    if energy_constraint_index:
        ...
        if self.counter > self.num_count:
...
def adapt_pspace(self):
    ...
    if hasattr(self, 'weight_type') and self.weight_type == 'ones':
...
OneSidedEnergy.adapt_pspace = adapt_pspace_energy
SystemEquations.assign_pspace = assign_pspace_system
SystemEquations.objective = objective_system
SystemEquations.jacobian = jacobian_system
SystemEquations.update_weights = update_weights
SystemEquations.adapt_pspace = adapt_pspace
