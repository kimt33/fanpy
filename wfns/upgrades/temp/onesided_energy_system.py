...
class OneSidedEnergySystem(SystemEquations):
    ...
    def objective(self, params):
        ...
        orig_num_eqns = OneSidedEnergySystem.num_eqns
        OneSidedEnergySystem.num_eqns = SystemEquations.num_eqns

        system_eqns = super().objective(params)
        if hasattr(self, 'print_energy') and self.print_energy:
            print("(Mid Optimization) Electronic Energy: {}".format(self.energy.params[0]))

        # patch back
        OneSidedEnergySystem.num_eqns = orig_num_eqns

        return np.sum(system_eqns ** 2) + self.energy.params[0] * self.energy_weight
    ...
    def gradient(self, params):
        ...
        # FIXME: monkey patched property
        # The property num_eqns need to be that of SystemEquations rather than the
        # OneSidedEnergySystem for SystemEquations.objective to function properly. This occurs
        # because the number of equations goes to 1 in OneSidedEnergySystem. We can add more
        # structure to differentiate between inherited and changed values, but this would require a
        # rewrite of the other parts of code. Since this problem occurs only in this module, we will
        # just patch in the fix here.
        temp = OneSidedEnergySystem.num_eqns
        OneSidedEnergySystem.num_eqns = SystemEquations.num_eqns
        ...
        system_eqns_jac, system_eqns, d_energy = super().jacobian(params, return_d_energy=True)
        system_eqns = system_eqns[:, np.newaxis]

        # patch back
        OneSidedEnergySystem.num_eqns = temp

        return 2 * np.sum(system_eqns * system_eqns_jac, axis=0) + d_energy * self.energy_weight
