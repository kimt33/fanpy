import numpy as np
from numpy.linalg import norm
from scipy.linalg import svd, qr
from scipy.sparse.linalg import lsmr
from scipy.optimize import OptimizeResult
from scipy._lib.six import string_types

from scipy.optimize._lsq.common import (
    solve_lsq_trust_region, solve_trust_region_2d, minimize_quadratic_1d, build_quadratic_1d,
    evaluate_quadratic, right_multiplied_operator, compute_grad, compute_jac_scale,
    update_tr_radius, scale_for_robust_loss_function, print_header_nonlinear,
    print_iteration_nonlinear, find_active_constraints, make_strictly_feasible,
    regularized_lsq_operator, CL_scaling_vector
)
import scipy.optimize._lsq.trf
from scipy.optimize._lsq.trf import select_step
from scipy.optimize._lsq.least_squares import TERMINATION_MESSAGES

from wfns.backend import math_tools
from wfns.solver.wrappers import wrap_scipy


def check_termination(dF, F, dx_norm, x_norm, ratio, ftol, xtol):
    """Check termination condition for nonlinear least squares."""
    ftol_satisfied = dF < ftol * F and ratio > 0.25
    xtol_satisfied = dx_norm < xtol * (xtol + x_norm)

    if ftol_satisfied and xtol_satisfied:
        return 4
    elif ftol_satisfied:
        return 2
    elif xtol_satisfied:
        return 3
    else:
        return None


def trf_bounds_fanpy(fun, jac, x0, f0, J0, lb, ub, ftol, xtol, gtol, max_nfev,
                     x_scale, loss_function, tr_solver, tr_options, verbose):
    print('X'*99)
    schrodinger = tr_options.pop('schrodinger')
    if (
        schrodinger.wfn in schrodinger.param_selection._masks_container_params and
        schrodinger.param_selection._masks_container_params[schrodinger.wfn].size
    ):
        wfn = schrodinger.wfn
        param_indices = schrodinger.param_selection._masks_objective_params[wfn]
        wfn_indices = schrodinger.param_selection._masks_container_params[wfn]
    else:
        wfn = None
    if (
        schrodinger.ham in schrodinger.param_selection._masks_container_params and
        schrodinger.param_selection._masks_container_params[schrodinger.ham].size
    ):
        ham = schrodinger.ham
        ham.update_prev_params = False
    else:
        ham = None
    termination_counter = 0
    counter_limit = tr_options.pop('counter_limit', 10)
    cost_tol = tr_options.pop('cost_tol', 1e-10)

    x = x0.copy()

    f = f0
    f_true = f.copy()
    nfev = 1

    J = J0
    njev = 1
    m, n = J.shape

    if loss_function is not None:
        rho = loss_function(f)
        cost = 0.5 * np.sum(rho[0])
        J, f = scale_for_robust_loss_function(J, f, rho)
    else:
        cost = 0.5 * np.dot(f, f)

    g = compute_grad(J, f)

    jac_scale = isinstance(x_scale, string_types) and x_scale == 'jac'
    if jac_scale:
        scale, scale_inv = compute_jac_scale(J)
    else:
        scale, scale_inv = x_scale, 1 / x_scale

    v, dv = CL_scaling_vector(x, g, lb, ub)
    v[dv != 0] *= scale_inv[dv != 0]
    Delta = norm(x0 * scale_inv / v**0.5)
    if Delta == 0:
        Delta = 1.0

    g_norm = norm(g * v, ord=np.inf)

    f_augmented = np.zeros((m + n))
    if tr_solver == 'exact':
        J_augmented = np.empty((m + n, n))
    elif tr_solver == 'lsmr':
        reg_term = 0.0
        regularize = tr_options.pop('regularize', True)

    if max_nfev is None:
        max_nfev = x0.size * 100

    alpha = 0.0  # "Levenberg-Marquardt" parameter

    termination_status = None
    iteration = 0
    step_norm = None
    actual_reduction = None

    if verbose == 2:
        print_header_nonlinear()

    while True:
        v, dv = CL_scaling_vector(x, g, lb, ub)

        g_norm = norm(g * v, ord=np.inf)

        while g_norm < gtol or cost < cost_tol:
            if verbose == 2:
                print(
                    "(Mid Optimization) Electronic Energy: {}".format(schrodinger.print_queue['energy'])
                )
                print(
                    "(Mid Optimization) Cost (without constraints): "
                    "{}".format(schrodinger.print_queue['cost'])
                )
                if 'constraint' in schrodinger.print_queue:
                    print(
                        "(Mid Optimization) Constraint Cost: "
                        "{}".format(schrodinger.print_queue['constraint'])
                    )

            if g_norm < gtol:
                termination_status = 1
            # elif cost < cost_tol:
            else:
                termination_status = 5
            termination_counter += 1

            if termination_counter >= counter_limit:
                break

            # weigh equations
            if hasattr(schrodinger, 'adaptive_weights') and schrodinger.adaptive_weights:
                schrodinger.update_weights()

            # Update objective
            schrodinger.adapt_pspace()

            # Update normalization pspace
            wfn.update_pspace_norm()
            print(np.linalg.norm(ham.params), 'ham_norm')

            # Reset trust region
            Delta = norm(x * scale_inv)
            alpha = 0.0

            f = fun(x)
            f_true = f.copy()
            # Normalize wavefunction parameters
            # NOTE: `fun` normalizes the wavefunction if it's being optimized
            if wfn:
                x[param_indices] = wfn.params.flatten()[wfn_indices]

            if loss_function is not None:
                cost = loss_function(f, cost_only=True)
            else:
                cost = 0.5 * np.dot(f, f)

            J = jac(x, f)
            njev += 1
            m, n = J.shape

            if loss_function is not None:
                rho = loss_function(f)
                J, f = scale_for_robust_loss_function(J, f, rho)

            g = compute_grad(J, f)
            g_norm = norm(g, ord=np.inf)

            f_augmented = np.zeros((m + n))
            if tr_solver == 'exact':
                J_augmented = np.empty((m + n, n))
            elif tr_solver == 'lsmr':
                reg_term = 0.0
                regularize = tr_options.pop('regularize', True)

            if jac_scale:
                scale, scale_inv = compute_jac_scale(J, scale_inv)

        print(termination_counter, termination_status, 'x'*99)
        if verbose == 2:
            print(
                "(Mid Optimization) Electronic Energy: {}".format(schrodinger.print_queue['energy'])
            )
            print(
                "(Mid Optimization) Cost (without constraints): "
                "{}".format(schrodinger.print_queue['cost'])
            )
            if 'constraint' in schrodinger.print_queue:
                print(
                    "(Mid Optimization) Constraint Cost: "
                    "{}".format(schrodinger.print_queue['constraint'])
                )
            print_iteration_nonlinear(iteration, nfev, cost, actual_reduction,
                                      step_norm, g_norm)

        if nfev == max_nfev or termination_counter >= counter_limit:
            break

        # Now compute variables in "hat" space. Here we also account for
        # scaling introduced by `x_scale` parameter. This part is a bit tricky,
        # you have to write down the formulas and see how the trust-region
        # problem is formulated when the two types of scaling are applied.
        # The idea is that first we apply `x_scale` and then apply Coleman-Li
        # approach in the new variables.

        # v is recomputed in the variables after applying `x_scale`, note that
        # components which were identically 1 not affected.
        v[dv != 0] *= scale_inv[dv != 0]

        # Here we apply two types of scaling.
        d = v**0.5 * scale

        # C = diag(g * scale) Jv
        diag_h = g * dv * scale

        # After all this were done, we continue normally.

        # "hat" gradient.
        g_h = d * g

        f_augmented[:m] = f
        if tr_solver == 'exact':
            J_augmented[:m] = J * d
            J_h = J_augmented[:m]  # Memory view.
            J_augmented[m:] = np.diag(diag_h**0.5)
            U, s, V = svd(J_augmented, full_matrices=False)
            V = V.T
            uf = U.T.dot(f_augmented)
        elif tr_solver == 'lsmr':
            J_h = right_multiplied_operator(J, d)

            if regularize:
                a, b = build_quadratic_1d(J_h, g_h, -g_h, diag=diag_h)
                to_tr = Delta / norm(g_h)
                ag_value = minimize_quadratic_1d(a, b, 0, to_tr)[1]
                reg_term = -ag_value / Delta**2

            lsmr_op = regularized_lsq_operator(J_h, (diag_h + reg_term)**0.5)
            gn_h = lsmr(lsmr_op, f_augmented, **tr_options)[0]
            S = np.vstack((g_h, gn_h)).T
            S, _ = qr(S, mode='economic')
            JS = J_h.dot(S)  # LinearOperator does dot too.
            B_S = np.dot(JS.T, JS) + np.dot(S.T * diag_h, S)
            g_S = S.T.dot(g_h)

        # theta controls step back step ratio from the bounds.
        theta = max(0.995, 1 - g_norm)

        actual_reduction = -1
        print('y'*99)
        while actual_reduction <= 0 and nfev < max_nfev:
            if tr_solver == 'exact':
                p_h, alpha, n_iter = solve_lsq_trust_region(
                    n, m, uf, s, V, Delta, initial_alpha=alpha)
            elif tr_solver == 'lsmr':
                p_S, _ = solve_trust_region_2d(B_S, g_S, Delta)
                p_h = S.dot(p_S)

            p = d * p_h  # Trust-region solution in the original space.
            step, step_h, predicted_reduction = select_step(
                x, J_h, diag_h, g_h, p, p_h, d, Delta, lb, ub, theta)

            x_new = make_strictly_feasible(x + step, lb, ub, rstep=0)
            f_new = fun(x_new)
            nfev += 1

            # Normalize wavefunction parameters
            # NOTE: `fun` normalizes the wavefunction if it's being optimized
            # NOTE: not necessary?
            # if wfn:
            #     x_new[param_indices] = wfn.params.flatten()[wfn_indices]

            step_h_norm = norm(step_h)

            if not np.all(np.isfinite(f_new)):
                Delta = 0.25 * step_h_norm
                continue

            # Usual trust-region step quality estimation.
            if loss_function is not None:
                cost_new = loss_function(f_new, cost_only=True)
            else:
                cost_new = 0.5 * np.dot(f_new, f_new)
            actual_reduction = cost - cost_new
            # Correction term is specific to the algorithm,
            # vanishes in unbounded case.
            correction = 0.5 * np.dot(step_h * diag_h, step_h)

            Delta_new, ratio = update_tr_radius(
                Delta, actual_reduction, predicted_reduction,
                step_h_norm, step_h_norm > 0.95 * Delta)

            step_norm = norm(step)
            termination_status = check_termination(
                actual_reduction, cost, step_norm, norm(x), ratio, ftol, xtol)
            if termination_status is not None:
                termination_counter += 1
                break

            alpha *= Delta / Delta_new
            Delta = Delta_new

        print('z'*99)
        # Increment counter for weight adaptation
        if hasattr(schrodinger, 'counter'):
            schrodinger.counter += 1
        if actual_reduction > 0:
            # Update parameters of Hamiltonian
            if ham:
                params_diff = ham.params - ham._prev_params
                unitary = math_tools.unitary_matrix(params_diff)
                ham._prev_params = ham.params.copy()
                ham._prev_unitary = ham._prev_unitary.dot(unitary)

            x = x_new
        else:
            step_norm = 0
            actual_reduction = 0

        # weigh equations
        if hasattr(schrodinger, 'adaptive_weights') and schrodinger.adaptive_weights:
            schrodinger.update_weights()
        print(f[-1], schrodinger.eqn_weights[-1], 'weights')

        # Update objective
        schrodinger.adapt_pspace()

        # Update normalization pspace
        wfn.update_pspace_norm()
        schrodinger.refwfn = list(wfn.pspace_norm)
        print(np.linalg.norm(ham.params), 'ham_norm')

        # reset trust region
        Delta = norm(x * scale_inv)
        alpha = 0.0

        f = fun(x)
        f_true = f.copy()
        # Normalize wavefunction parameters
        # NOTE: `fun` normalizes the wavefunction if it's being optimized
        if wfn:
            x[param_indices] = wfn.params.flatten()[wfn_indices]

        if loss_function is not None:
            cost = loss_function(f, cost_only=True)
        else:
            cost = 0.5 * np.dot(f, f)

        J = jac(x, f)
        njev += 1
        m, n = J.shape

        if loss_function is not None:
            rho = loss_function(f)
            J, f = scale_for_robust_loss_function(J, f, rho)

        g = compute_grad(J, f)

        f_augmented = np.zeros((m + n))
        if tr_solver == 'exact':
            J_augmented = np.empty((m + n, n))
        elif tr_solver == 'lsmr':
            reg_term = 0.0
            regularize = tr_options.pop('regularize', True)

        if jac_scale:
            scale, scale_inv = compute_jac_scale(J, scale_inv)

        iteration += 1
        if termination_status is None:
            termination_counter = 0

    if termination_status is None:
        termination_status = 0

    active_mask = find_active_constraints(x, lb, ub, rtol=xtol)
    return OptimizeResult(
        x=x, cost=cost, fun=f_true, jac=J, grad=g, optimality=g_norm,
        active_mask=active_mask, nfev=nfev, njev=njev,
        status=termination_status)


def trf_no_bounds_fanpy(fun, jac, x0, f0, J0, ftol, xtol, gtol, max_nfev,
                        x_scale, loss_function, tr_solver, tr_options, verbose):
    schrodinger = tr_options.pop('schrodinger')
    if (
        schrodinger.wfn in schrodinger.param_selection._masks_container_params and
        schrodinger.param_selection._masks_container_params[schrodinger.wfn].size
    ):
        wfn = schrodinger.wfn
        param_indices = schrodinger.param_selection._masks_objective_params[wfn]
        wfn_indices = schrodinger.param_selection._masks_container_params[wfn]
    else:
        wfn = None
    if (
        schrodinger.ham in schrodinger.param_selection._masks_container_params and
        schrodinger.param_selection._masks_container_params[schrodinger.ham].size
    ):
        ham = schrodinger.ham
        ham.update_prev_params = False
    else:
        ham = None
    termination_counter = 0
    counter_limit = tr_options.pop('counter_limit', 10)
    cost_tol = tr_options.pop('cost_tol', 1e-10)

    x = x0.copy()

    f = f0
    f_true = f.copy()
    nfev = 1

    J = J0
    njev = 1
    m, n = J.shape

    if loss_function is not None:
        rho = loss_function(f)
        cost = 0.5 * np.sum(rho[0])
        J, f = scale_for_robust_loss_function(J, f, rho)
    else:
        cost = 0.5 * np.dot(f, f)

    g = compute_grad(J, f)

    jac_scale = isinstance(x_scale, string_types) and x_scale == 'jac'
    if jac_scale:
        scale, scale_inv = compute_jac_scale(J)
    else:
        scale, scale_inv = x_scale, 1 / x_scale

    Delta = norm(x0 * scale_inv)
    if Delta == 0:
        Delta = 1.0

    if tr_solver == 'lsmr':
        reg_term = 0
        damp = tr_options.pop('damp', 0.0)
        regularize = tr_options.pop('regularize', True)

    if max_nfev is None:
        max_nfev = x0.size * 100

    alpha = 0.0  # "Levenberg-Marquardt" parameter

    termination_status = None
    iteration = 0
    step_norm = None
    actual_reduction = None

    if verbose == 2:
        print_header_nonlinear()

    while True:
        g_norm = norm(g, ord=np.inf)
        while g_norm < gtol or cost < cost_tol:
            if verbose == 2:
                print(
                    "(Mid Optimization) Electronic Energy: {}".format(schrodinger.print_queue['energy'])
                )
                print(
                    "(Mid Optimization) Cost (without constraints): "
                    "{}".format(schrodinger.print_queue['cost'])
                )
                if 'constraint' in schrodinger.print_queue:
                    print(
                        "(Mid Optimization) Constraint Cost: "
                        "{}".format(schrodinger.print_queue['constraint'])
                    )

            if g_norm < gtol:
                termination_status = 1
            # elif cost < cost_tol:
            else:
                termination_status = 5
            termination_counter += 1

            if termination_counter >= counter_limit:
                break

            # Update objective
            schrodinger.adapt_pspace()

            # Update normalization pspace
            wfn.update_pspace_norm()
            print(np.linalg.norm(ham.params), 'ham_norm')

            # Reset trust region
            Delta = norm(x * scale_inv)
            alpha = 0.0

            f = fun(x)
            f_true = f.copy()
            # Normalize wavefunction parameters
            # NOTE: `fun` normalizes the wavefunction if it's being optimized
            if wfn:
                x[param_indices] = wfn.params.flatten()[wfn_indices]

            if loss_function is not None:
                cost = loss_function(f, cost_only=True)
            else:
                cost = 0.5 * np.dot(f, f)

            J = jac(x, f)
            njev += 1
            m, n = J.shape

            if loss_function is not None:
                rho = loss_function(f)
                J, f = scale_for_robust_loss_function(J, f, rho)

            g = compute_grad(J, f)
            g_norm = norm(g, ord=np.inf)

            if jac_scale:
                scale, scale_inv = compute_jac_scale(J, scale_inv)

        print(termination_counter, termination_status, 'x'*99)
        if verbose == 2:
            print(
                "(Mid Optimization) Electronic Energy: {}".format(schrodinger.print_queue['energy'])
            )
            print(
                "(Mid Optimization) Cost (without constraints): "
                "{}".format(schrodinger.print_queue['cost'])
            )
            if 'constraint' in schrodinger.print_queue:
                print(
                    "(Mid Optimization) Constraint Cost: "
                    "{}".format(schrodinger.print_queue['constraint'])
                )
            print_iteration_nonlinear(iteration, nfev, cost, actual_reduction,
                                      step_norm, g_norm)

        if nfev == max_nfev or termination_counter >= counter_limit:
            break

        d = scale
        g_h = d * g

        if tr_solver == 'exact':
            J_h = J * d
            U, s, V = svd(J_h, full_matrices=False)
            V = V.T
            uf = U.T.dot(f)
        elif tr_solver == 'lsmr':
            J_h = right_multiplied_operator(J, d)

            if regularize:
                a, b = build_quadratic_1d(J_h, g_h, -g_h)
                to_tr = Delta / norm(g_h)
                ag_value = minimize_quadratic_1d(a, b, 0, to_tr)[1]
                reg_term = -ag_value / Delta**2

            damp_full = (damp**2 + reg_term)**0.5
            gn_h = lsmr(J_h, f, damp=damp_full, **tr_options)[0]
            S = np.vstack((g_h, gn_h)).T
            S, _ = qr(S, mode='economic')
            JS = J_h.dot(S)
            B_S = np.dot(JS.T, JS)
            g_S = S.T.dot(g_h)

        actual_reduction = -1
        print('y'*99)
        while actual_reduction <= 0 and nfev < max_nfev:
            if tr_solver == 'exact':
                step_h, alpha, n_iter = solve_lsq_trust_region(
                    n, m, uf, s, V, Delta, initial_alpha=alpha)
            elif tr_solver == 'lsmr':
                p_S, _ = solve_trust_region_2d(B_S, g_S, Delta)
                step_h = S.dot(p_S)

            predicted_reduction = -evaluate_quadratic(J_h, g_h, step_h)
            step = d * step_h
            x_new = x + step
            f_new = fun(x_new)
            nfev += 1

            # Normalize wavefunction parameters
            # NOTE: `fun` normalizes the wavefunction if it's being optimized
            # NOTE: not necessary?
            # if wfn:
            #     x_new[param_indices] = wfn.params.flatten()[wfn_indices]

            step_h_norm = norm(step_h)

            if not np.all(np.isfinite(f_new)):
                Delta = 0.25 * step_h_norm
                continue

            # Usual trust-region step quality estimation.
            if loss_function is not None:
                cost_new = loss_function(f_new, cost_only=True)
            else:
                cost_new = 0.5 * np.dot(f_new, f_new)
            actual_reduction = cost - cost_new

            Delta_new, ratio = update_tr_radius(
                Delta, actual_reduction, predicted_reduction,
                step_h_norm, step_h_norm > 0.95 * Delta)

            step_norm = norm(step)
            termination_status = check_termination(
                actual_reduction, cost, step_norm, norm(x), ratio, ftol, xtol)
            if termination_status is not None:
                termination_counter += 1
                break

            alpha *= Delta / Delta_new
            Delta = Delta_new

        print('z'*99)
        # Increment counter for weight adaptation
        if hasattr(schrodinger, 'counter'):
            schrodinger.counter += 1
        if actual_reduction > 0:
            # Update parameters of Hamiltonian
            if ham:
                params_diff = ham.params - ham._prev_params
                unitary = math_tools.unitary_matrix(params_diff)
                ham._prev_params = ham.params.copy()
                ham._prev_unitary = ham._prev_unitary.dot(unitary)

            x = x_new
        else:
            step_norm = 0
            actual_reduction = 0

        # Update objective
        schrodinger.adapt_pspace()

        # Update normalization pspace
        wfn.update_pspace_norm()
        print(np.linalg.norm(ham.params), 'ham_norm')

        # reset trust region
        Delta = norm(x * scale_inv)
        alpha = 0.0

        f = fun(x)
        f_true = f.copy()
        # Normalize wavefunction parameters
        # NOTE: `fun` normalizes the wavefunction if it's being optimized
        if wfn:
            x[param_indices] = wfn.params.flatten()[wfn_indices]

        if loss_function is not None:
            cost = loss_function(f, cost_only=True)
        else:
            cost = 0.5 * np.dot(f, f)

        J = jac(x, f)
        njev += 1
        m, n = J.shape

        if loss_function is not None:
            rho = loss_function(f)
            J, f = scale_for_robust_loss_function(J, f, rho)

        g = compute_grad(J, f)

        if jac_scale:
            scale, scale_inv = compute_jac_scale(J, scale_inv)

        iteration += 1
        if termination_status is None:
            termination_counter = 0

    if termination_status is None:
        termination_status = 0

    active_mask = np.zeros_like(x)
    return OptimizeResult(
        x=x, cost=cost, fun=f_true, jac=J, grad=g, optimality=g_norm,
        active_mask=active_mask, nfev=nfev, njev=njev,
        status=termination_status)


def least_squares(objective, energy_bounds=(-np.inf, np.inf), save_file="", **kwargs):
    from scipy.optimize import least_squares as solver

    if kwargs == {}:
        kwargs = {
            "xtol": 1.0e-8,
            "ftol": 1.0e-8,
            "gtol": 1.0e-8,
            "max_nfev": 1000 * objective.params.size,
            "jac": objective.jacobian,
        }
    try:
        kwargs['tr_options'].setdefault('schrodinger', objective)
        kwargs['tr_options'].setdefault('cost_tol', 1e-10)
        kwargs['tr_options'].setdefault('counter_limit', 10)
    except KeyError:
        kwargs['tr_options'] = {'schrodinger': objective, 'cost_tol': 1e-10, 'counter_limit': 10}

    objective.print_energy = False
    objective.print_queue = {}

    low_bounds = -2 * np.ones(objective.params.size)
    upp_bounds = 2 * np.ones(objective.params.size)
    if objective.energy_type == 'variable':
        low_bounds[-1] = energy_bounds[0]
        upp_bounds[-1] = energy_bounds[1]

    output = wrap_scipy(solver)(objective, bounds=(low_bounds, upp_bounds), save_file=save_file, verbose=2, **kwargs)
    output["energy"] = objective.energy.params[0]
    output["residuals"] = output["internal"].fun
    output["cost"] = output["internal"].cost
    return output


scipy.optimize._lsq.trf.trf_bounds = trf_bounds_fanpy
scipy.optimize._lsq.trf.trf_no_bounds = trf_no_bounds_fanpy
TERMINATION_MESSAGES[5] = (
    "`cost_tol` termination condition is satisfied."
)
