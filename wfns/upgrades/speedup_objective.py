...
from wfns.upgrades.cext_apg import _olp_deriv_internal_ap1rog
from wfns.upgrades.cext_apg_parallel import _olp_deriv_internal_apig
from wfns.upgrades.cext_apg_parallel2 import _olp_deriv_internal
import wfns.backend.slater as slater
from wfns.ham.restricted_chemical import RestrictedChemicalHamiltonian
from wfns.objective.schrodinger.base import BaseSchrodinger
from wfns.upgrades.cext_objective import get_energy_one_proj_deriv
...
def parallel_energy_deriv(sds_objective):
    ...
    if isinstance(objective.wfn, BaseGeminal):
        import wfns.upgrades.speedup_apg
...
def get_energy_one_proj(self, pspace, deriv=None, parallel=None):
    ...
    if deriv is not None:
        ...
        output = np.zeros(self.params.size)
        try:
            output[self.param_selection._masks_objective_params[self.wfn]] = d_energy[:self.wfn.nparams][self.param_selection._masks_container_params[self.wfn]]
        except KeyError:
            pass
        try:
            output[self.param_selection._masks_objective_params[self.ham]] = d_energy[self.wfn.nparams:][self.param_selection._masks_container_params[self.ham]]
        except KeyError:
            pass
        return output
